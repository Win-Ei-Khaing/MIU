public class HelpFindCat {
	public static boolean findCat(boolean[][] room, int lr, int hr, int lc, int hc) {
		...
		if (lr == hr && lc == hc) {
			...
			if (!found)
				found = findCat(room, lr, hr, mc + 1, hc);
		} else if (lc == hc) {
			int mr = (lr + hr) / 2;
			found = findCat(room, lr, mr, lc, hc);
			if (!found)
				found = findCat(room, mr + 1, hr, lc, hc);
		} else {
			int mc = (lc + hc) / 2;
			int mr = (lr + hr) / 2;
			found = findCat(room, lr, mr, lc, mc);
			if (!found)
				found = findCat(room, mr + 1, hr, lc, mc);
			if (!found)
				found = findCat(room, lr, mr, mc + 1, hc);
			if (!found)
				found = findCat(room, mr + 1, hr, mc + 1, hc);
		}
		return found;
	}
