...
public class LambdaLibrary {
	...
	public static Predicate<CheckoutRecord> OVERDUEBOOK=r->r.getCheckoutEntries()
			.stream()
			.filter(x->!x.getCopy().isAvailable())
			.filter(x->Date.valueOf(x.getDueDate()).before(Calendar.getInstance().getTime())).findAny().isPresent();
