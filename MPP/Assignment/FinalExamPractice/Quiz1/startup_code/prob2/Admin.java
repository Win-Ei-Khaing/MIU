...
public class Admin {
	...
	public static List<String> getPhoneNumsByStream(LibraryMember[] members, LendingItem item) {
		...
		/*phoneNums = Stream.of(members).filter(m->m.getCheckoutRecord().getCheckoutEntryList()
				.stream().anyMatch(x->x.getLendingItem().equals(item)))
				.map(m->m.getPhone())
				.sorted().collect(Collectors.toList());*/
		
		phoneNums = Stream.of(members)
				.filter((m->m.getCheckoutRecord().getCheckoutEntryList().stream().map(e->e.getLendingItem()).collect(Collectors.toList()).contains(item)))
				.map(m->m.getPhone())
				.sorted()
				.collect(Collectors.toList());
		return phoneNums;
