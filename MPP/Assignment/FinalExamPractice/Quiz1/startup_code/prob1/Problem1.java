...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		...
		return Stream.concat(list1.stream(), list2.stream().filter(x->!list1.contains(x))).collect(Collectors.toList());
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		//implemented by Win
		return list.stream().filter(c->c.getCity().equals("Fairfield")).map(c->c.getName()).collect(Collectors.toList());
	}
