...
public class EmployeeInfoBetter {
	...
	public void sort(List<Employee> emps, final SortMethod method) {
		if(method == SortMethod.BYNAME) {
			//System.out.println(emps.stream().sorted(Comparator.comparing(byName).thenComparing(bySalary)).collect(Collectors.toList()));
			Collections.sort(emps, Comparator.comparing((Employee e)->e.getName()).thenComparing(e->e.getSalary()));
		} else {
