...
public class EmployeeInfo {
	...
	//by win
	static void sortByWin(List<Employee> emps, SortMethod sMethod) {
		Comparator<Employee> eComparator = (e1, e2)->{
			if(sMethod==SortMethod.BYNAME)
		...
		Collections.sort(emps, eComparator);
	}
...
//		emps.add(new Employee("Andy", 60000));
		...
		Employee joe2 = new Employee("Aoe", 200000);
		List<Employee> list = Arrays.asList(joe, joe2);
//		Collections.sort(list, new EmployeeNameComparator());
//		System.out.println(list);
		sortByWin(list, SortMethod.BYNAME);
		System.out.println(list);
		sortByWin(list, SortMethod.BYSALARY);
		System.out.println(list);
