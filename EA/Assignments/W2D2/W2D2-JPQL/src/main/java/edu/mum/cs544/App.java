...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Airline> airlines = em.createQuery("select distinct f.airline from Flight f where f.airplane.model = 'A380'", Airline.class).getResultList();
        //List<Airline> airlines = em.createQuery("select distinct al from Airline al join al.flights f where f.airplane.model = 'A380'", Airline.class).getResultList();
        ...
        flights = em.createQuery("select f from Flight f where f.airplane.model='747' and f.airline.name<>'Star Alliance'", Flight.class).getResultList();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
            System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
