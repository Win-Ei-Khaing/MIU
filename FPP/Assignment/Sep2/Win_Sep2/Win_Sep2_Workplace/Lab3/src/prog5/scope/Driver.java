...
public class Driver {
	...
	Driver(){
		...
		while(!inputString.equalsIgnoreCase("N")){
			...
			else if(inputString.trim().startsWith("booYk")){  //add cd
				int len = "book".length();
	...
	void displayNumberOfEmployees() {
		//implement -- redo the implementation -- did by win
		int numOfEmployees=directory.getNumberOfBookstoreEmployees();
		if(numOfEmployees > -1)
			System.out.println("Number of employees is: " + numOfEmployees+" (Look for other console messages.)"+n+CONTINUE);
	...
	void addCD(String cdId){
		//implement-- redo the implementation -- did by win
		int cdIdNum=1;//hardcoded because userinput will be string
		if(directory.addNewCD(cdIdNum))
	...
	void displayNumberOfCDsInBookstore(){
		//implement-- redo the implementation -- did by win
		int cdNum=directory.getNumCDsInBookstore();
		System.out.println("Number of CD's is " + cdNum+" (Look for console messages.)"+n+CONTINUE);
